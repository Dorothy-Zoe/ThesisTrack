...
function uploadAndAnalyze(file, chapterNumber) {
  ...
  xhr.upload.addEventListener("progress", function(e) {
    if (e.lengthComputable) {
      ...
      updateUploadProgress(chapterNumber, percentComplete, "uploading")
    }
  })

  // Track download progress (for the analysis phase)
  xhr.addEventListener("progress", function(e) {
    if (e.lengthComputable) {
      const percentComplete = 50 + (e.loaded / e.total) * 50 // Assume analysis is second half
      updateUploadProgress(chapterNumber, percentComplete, "analyzing")
    }
  })
  ...
  xhr.onload = function() {
    if (xhr.status === 200) {
      try {
        ...
        if (data.success) {
          // Show completion state briefly before updating UI
          updateUploadProgress(chapterNumber, 100, "complete")
...
// NEW FUNCTION: Show uploading state with progress bar
function showUploadingState(chapterNumber, filename) {
  ...
  if (uploadArea) {
      ...
      <div class="uploading-state">
        ...
        <div class="uploading-content">
          ...
          <div class="uploading-text">
            ...
            <p class="upload-status">Starting upload...</p>
          </div>
        </div>
    ...
    cancelBtn.onclick = () => cancelUpload(chapterNumber)
    uploadArea.querySelector('.uploading-state').appendChild(cancelBtn)
  }
}

// NEW FUNCTION: Update upload progress
function updateUploadProgress(chapterNumber, progress, status) {
  const uploadArea = document.querySelector(`#chapter${chapterNumber}`)?.closest(".chapter-card")?.querySelector(".upload-area")
  if (!uploadArea) return

  const progressFill = uploadArea.querySelector('.upload-progress-fill')
  const progressText = uploadArea.querySelector('.upload-progress-text')
  ...
  if (statusText) {
    let statusMessage = "Starting upload..."
    if (status === "uploading") {
      statusMessage = "Uploading file..."
    } else if (status === "analyzing") {
      statusMessage = "AI analysis in progress..."
    } else if (status === "complete") {
      statusMessage = "Upload complete! Finalizing..."
    }
...
// NEW FUNCTION: Cancel upload
function cancelUpload(chapterNumber) {
  // In a real implementation, you would abort the XMLHttpRequest
...
function resetUploadState(chapterNumber) {
  ...
  if (uploadArea) {
    ...
    // Re-initialize the file input event listener
    fileInput.addEventListener("change", function (e) {
      if (this.files && this.files[0]) {
        uploadAndAnalyze(this.files[0], chapterNumber)
      }
    })
  }
}
...
function viewComprehensiveReport(chapterNumber, version) {
    ...
    const fetchPromises = [
        ...
        : Promise.resolve({ 
            success: false, 
            error: "Citation analysis only available for Chapter 5",
